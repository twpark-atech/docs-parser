...
class EmbeddingConfig:
    """EmbeddingProvider 설정 값 클래스.
    OpenAI 호환 /v1/embeddings 호출에 필요한 기본 정보와 운영 파라미터를 담습니다.
...
class EmbeddingProvider:
    ...
    def __init__(self, cfg: EmbeddingConfig):
        ...
        if base.endswith("/v1/embeddings"):
            ...
            self._endpoint = base + "/embeddings"
        else:
            self._endpoint = base + "/v1/embeddings"
    ...
    def embed(self, texts: List[str]) -> List[List[float]]:
        ...
        Raises:
            ...
            ValueError: Embedding API가 빈 embeddings를 반환하거나 응답 형식이 잘못된 경우.
        """
    ...
    def _embed_batch(self, inputs: List[str]) -> List[List[float]]:
        """단일 배치 입력을 /v1/embeddings로 호출해 embeddings를 반환합니다.
        ...
        def _call():
            ...
            # Backward compatibility for servers returning {"embeddings":[...]}
            embs = data.get("embeddings")
            if isinstance(embs, list) and embs:
                return embs

            raise ValueError(f"Embedding API returned empty embeddings: {data}")
