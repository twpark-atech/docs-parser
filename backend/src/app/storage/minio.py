...
class MinIOWriter:
    ...
    def list_keys(self, *, prefix: str) -> List[str]:
        """지정 prefix 하위 object key 목록을 반환합니다."""
        out: List[str] = []
        for obj in self._client.list_objects(self._cfg.bucket, prefix=prefix, recursive=True):
    ...
    def get_object_bytes(self, *, object_key: str) -> bytes:
        """지정 object key의 바이트를 읽어 반환합니다."""
        resp = self._client.get_object(self._cfg.bucket, object_key)
        try:
            return resp.read()
        finally:
            resp.close()
    ...
    def delete_object(self, *, object_key: str) -> None:
        """지정 object key를 삭제합니다."""
        self._client.remove_object(self._cfg.bucket, object_key)

    def delete_prefix(self, *, prefix: str) -> int:
        """지정 prefix 하위 object를 모두 삭제하고 삭제 개수를 반환합니다."""
        keys = self.list_keys(prefix=prefix)
        deleted = 0
        for key in keys:
            try:
                self.delete_object(object_key=key)
                deleted += 1
            except Exception:
                continue
        return deleted

    def presigned_get_url(self, *, object_key: str, expires_sec: int = 3600) -> str:
        """지정 object key의 presigned GET URL을 생성합니다."""
        exp = max(1, int(expires_sec or 3600))
